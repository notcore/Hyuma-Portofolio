...
export default function Home() {
  ...
  const isFormValid = () => {
    return jumlah.trim() !== '' && hasUploadedQR;
  };
      ...
      <div>
        ...
        <div className="max-w-[430px] mx-auto">
          ...
          <h1 className="font-semibold text-black text-2xl mx-5 mt-2 text-center">
            Upload <span className="font-bold text-red-600">QR</span> ke-sini untuk melakukan{" "}
            <span className="font-bold text-red-600">Transaksi</span> jual barang.
          </h1>
          <p className="text-slate-400 text-center">menjual barang lebih mudah dengan metode pembayaran QR otomatis.</p>

          <div className="grid grid-cols-1 mx-5 mt-10 w-auto">
            {/* Input Jumlah */}
            <label htmlFor="jumlah" className="text-xl text-slate-600 text-start">
            ...
            <input
              ...
              required
              className="border border-slate-300 text-slate-500 focus:border-slate-400 focus:outline-none p-2 pl-2 mt-2 mb-3 bg-slate-50 rounded-xl h-12"
              placeholder="Rp."
            />
            ...
            {!cameraAllowed ? (
              <div>
                ...
                >
                  {isCapturing ? "Memproses..." : "Upload QR untuk pembayaran disini"}
                </button>
              ...
              <div>
                ...
                {isCapturing ? (
                  <div className="border border-blue-300 bg-blue-50 text-blue-600 p-3 mt-2 mb-3 rounded-xl text-center">
                    menyiapkan menu upload... ({cameraAllowed ? "tunggu sebentar" : "mohon izinkn akses kamera agar bisa upload QR code"})
                  </div>
                ) : (
                  <button
            ...
            <div className="text-sm text-slate-500 mb-4">
              <div className={`flex items-center gap-2 ${jumlah ? 'text-green-600' : ''}`}>
                <span>•</span>
                <span>Jumlah harga {jumlah ? 'Terisi' : 'harus ditulis'}</span>
              </div>
              <div className={`flex items-center gap-2 ${hasUploadedQR ? 'text-green-600' : ''}`}>
                <span>•</span>
